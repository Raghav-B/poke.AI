...
static HWND GetArchiveParentHWND() { return s_parentHWND ? s_parentHWND : AfxGetApp()->m_pMainWnd->GetSafeHwnd(); }

static char Str_Tmp [2048];
...
static struct TempFiles
{
	struct TemporaryFile
	{
		TemporaryFile(const char* cat, const char* ext)
		{
			...
			while(n != start)
			{
				_snprintf(fname, 2048 - (fname - tempPath), "%s%04X%s", cat, n, ext);
...
// example output LogicalName:  "C:\games.zip|Metroid.gba"
// example output PhysicalName: "C:\Documents and Settings\User\Local Settings\Temp\VBA\rom7A37.gba"
...
restart:
	...
	while(true)
	{
		ArchiveFile archive (PhysicalName, LogicalName);
		if(!archive.IsCompressed())
		{
			if(archive.GetNumItems() > 0)
				return true;
			else
			{
				// failed or cancelled... backtrack to outermost archive if not already there
				char* div = NULL;
				if(LogicalName[strlen(LogicalName)-1] == '|')
				{
					LogicalName[strlen(LogicalName)-1] = '\0';
					div = strrchr(LogicalName, '|');
				}
				if(div && userSelected)
					goto restart;
				else
		...
		else
		{
			...
			userSelected |= archive.m_userMadeSelection;
			...
			_snprintf(LogicalName + strlen(LogicalName), 2048 - (strlen(LogicalName)+1), "|%s", archive.GetItemName(item));
		}
	}
}
