...
ArchiveFile::ArchiveFile(const char* filename, const char* displayFilename)
{
	assert(!s_formatInfos.empty());
	...
	if(displayFilename)
	{
		m_displayFilename = new char[strlen(displayFilename)+1];
		strcpy(m_displayFilename, displayFilename);
	}
