...
static inline u8 *HardwareToSoftwareAddress(HWAddressType address)
{
	...
	if (systemCartridgeType == IMAGE_GBA)
	{
		...
		if (!mask || (address & 0x00ffffff) > mask)
			return NULL;
		return &memoryMap[address >> 24].address[address & mask];
	}
	else
	{
...
struct MemoryRegion
{
	...
	unsigned int virtualIndex; // index into s_prevValues, s_curValues, and s_numChanges, valid after being initialized in
	                           // ResetMemoryRegions()
...
static MemoryRegion * *s_itemIndexToRegionPointer = 0; // used for random access into the memory list (trading memory size to
                                                       // get speed here, too bad it's so much memory), only valid when
                                                       // s_itemIndicesInvalid is false
static BOOL s_itemIndicesInvalid   = true; // if true, the link from listbox items to memory regions
                                           // (s_itemIndexToRegionPointer) and the link from memory regions to list box items
...
void ResetMemoryRegions()
{
	...
	static const int regionSearchGranularity = 0x100; // if this is too small, we'll waste time (in this function only), but if
	                                                  // any region in RAM isn't evenly divisible by this, we might crash.
...
int DeactivateRegion(MemoryRegion &region, MemoryList::iterator &iter, HWAddressType hardwareAddress, unsigned int size)
{
	...
	else //if(hardwareAddress > region.hardwareAddress && hardwareAddress + size < region.hardwareAddress + region.size)
	{
		...
		{ region.hardwareAddress + eraseSize, region.size - eraseSize, region.softwareAddress + eraseSize, region.virtualIndex + eraseSize };
		region.size = hardwareAddress - region.hardwareAddress;
...
void CalculateItemIndices(int itemSize)
{
	...
	for (MemoryList::iterator iter = s_activeMemoryRegions.begin(); iter != s_activeMemoryRegions.end(); ++iter)
	{
		...
		int startSkipSize  = ((unsigned int)(itemSize - (unsigned int)region.hardwareAddress)) % itemSize; // FIXME: is this
		                                                                                                   // still ok?
...
template <typename T>
inline bool LessCmp(T x, T y, T i)        { return x < y; }
template <typename T>
inline bool MoreCmp(T x, T y, T i)        { return x > y; }
template <typename T>
inline bool LessEqualCmp(T x, T y, T i)   { return x <= y; }
template <typename T>
inline bool MoreEqualCmp(T x, T y, T i)   { return x >= y; }
template <typename T>
inline bool EqualCmp(T x, T y, T i)       { return x == y; }
template <typename T>
inline bool UnequalCmp(T x, T y, T i)     { return x != y; }
template <typename T>
inline bool DiffByCmp(T x, T y, T p)      { return x - y == p || y - x == p; }
template <typename T>
inline bool ModIsCmp(T x, T y, T p)       { return p && x % p == y; }
...
void prune(char c, char o, char t, int v, int p)
{
	...
	{ \
	case '<': \
		DO_SEARCH_2(LessCmp, sf); break; \
	case '>': \
		DO_SEARCH_2(MoreCmp, sf); break; \
	case '=': \
		DO_SEARCH_2(EqualCmp, sf); break; \
	case '!': \
		DO_SEARCH_2(UnequalCmp, sf); break; \
	case 'l': \
		DO_SEARCH_2(LessEqualCmp, sf); break; \
	case 'm': \
		DO_SEARCH_2(MoreEqualCmp, sf); break; \
	case 'd': \
		DO_SEARCH_2(DiffByCmp, sf); break; \
	case '%': \
		DO_SEARCH_2(ModIsCmp, sf); break; \
	default: \
		assert(!"Invalid operator for this search type."); break; \
	}
...
int ReadControlInt(int controlID, bool forceHex, BOOL &success)
{
	...
	if (!ok)
	{
		...
		{
			for (int i = 0; Str_Tmp[i]; i++)
			{
				if (toupper(Str_Tmp[i]) == 'O')
					Str_Tmp[i] = '0';
			}
...
bool IsSatisfied(int itemIndex)
{
	...
	case 'r':
		DO_SEARCH(CompareRelativeAtItem); break;
	case 's':
		DO_SEARCH(CompareSpecificAtItem); break;
	...
	case 'a':
		DO_SEARCH(CompareAddressAtItem); break;
	...
	case 'n':
		DO_SEARCH(CompareChangesAtItem); break;
	}
	return false;
}
...
void reset_address_info()
{
	...
	EnterCriticalSection(&s_activeMemoryRegionsCS);
	s_activeMemoryRegionsBackup.clear(); // not necessary, but we'll take the time hit here instead of at the next thing that
	                                     // sets up an undo
	LeaveCriticalSection(&s_activeMemoryRegionsCS);
	if (s_prevValues)
...
void signal_new_size()
{
	...
	unsigned int oldSelectionAddr  = CALL_WITH_T_SIZE_TYPES_1(GetHardwareAddressFromItemIndex,
	                                                          rs_last_type_size,
	                                                          rs_t == 's',
	                                                          rs_last_no_misalign,
	                                                          oldSelectionIndex);
...
LRESULT CALLBACK RamSearchProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		SetWindowPos(hDlg, NULL, regQueryDwordValue("ramSearchX", 0), regQueryDwordValue("ramSearchY",
		                                                                                 0), NULL, NULL, SWP_NOSIZE | SWP_NOZORDER | SWP_SHOWWINDOW);
		switch (rs_o)
		{
		case '<':
	...
	case WM_NOTIFY:
	{
		...
		case LVN_GETDISPINFO:
		{
			...
			case 1:
			{
				...
				case 'b':
				default:
					sprintf(num, formatString, rs_t == 's' ? (char)(i & 0xff) : (unsigned char)(i & 0xff)); break;
				case 'w':
					sprintf(num, formatString, rs_t == 's' ? (short)(i & 0xffff) : (unsigned short)(i & 0xffff)); break;
				case 'd':
					sprintf(num, formatString, rs_t == 's' ? (long)(i & 0xffffffff) : (unsigned long)(i & 0xffffffff)); break;
				}
				Item->item.pszText = num;
			}   return true;
			case 2:
		...
		case NM_CUSTOMDRAW:
		{
			...
			//case LVN_ODCACHEHINT: //Copied this bit from the MSDN virtual listbox code sample. Eventually it should probably
			// do something.
			//{
			//	LPNMLVCACHEHINT   lpCacheHint = (LPNMLVCACHEHINT)lParam;
			//	return 0;
			//}
			//case LVN_ODFINDITEM: //Copied this bit from the MSDN virtual listbox code sample. Eventually it should probably do
			// something.
			//{
...
void init_list_box(HWND Box, const char *Strs[], int numColumns, int *columnWidths) //initializes the ram search and/or ram
                                                                                    // watch listbox
{
	LVCOLUMN Col;
